...
(function (global, factory) {
  ...
  EZUIPlayer.prototype.getRealUrl = function (playParams) {
    ...
    if (playParams && playParams.hasOwnProperty("decoderPath")) {
      if (
        playParams &&
        playParams.hasOwnProperty("userName") &&
        playParams.hasOwnProperty("password")
      ) {
          ...
          var apiSuccess = function (data) {
            if (data.code == 200 || data.retcode == 0) {
              ...
              realUrl +=
                ...
                data.data.localIp +
                ":" +
                data.data.wssLocalPort +
                "/" +
                "10" +
                (playParams.url.indexOf("hd") === -1 ? "2" : "1");
              // 执行设备授权
              capabilitiesUrl =
                apiDomain +
                "/jssdk/ezopen/sessionLogin/capabilities?ip=" +
                data.data.localIp +
                "&username=" +
                playParams.userName;
              var capabilitiesSuccess = function (xmlDoc, textStatus, xhr) {
                ...
                var isIrreversible =
                  isIrreversibleReg.exec(xmlDoc)[1] == "true";
                ...
                $.ajax({
                  ...
                  success: function (data) {
                    ...
                    szWebsocketSessionID = data.WebSession.split("=")[1].split(
                      ";"
                    )[0];
              ...
              var deviceSerialReg = /[a-zA-Z0-9]{9}\/[0-9]{0,2}\./;
              var deviceSerial = playParams.url
                .match(deviceSerialReg)[0]
                .split("/")[0];
        ...
        var getRealUrlPromise = function (resolve, reject, ezopenURL) {
          ...
          if (!/^ezopen:\/\//.test(ezopenURL)) {
            ...
            var apiSuccess = function (data) {
              if (data.code == 200 || data.retcode == 0) {
                ...
                if (data.ext && data.ext.token) {
                  ...
                  if (type === "live") {
                    realUrl =
                      realUrl + "&auth=1&biz=4&cln=100" + "&ssn=" + stream;
                  } else {
                    ...
                    function recAPISuccess(data) {
                      if (data.code == 200) {
                        ...
                        if (data.data && data.data.length > 0) {
                          ...
                          resolve(
                            JSON.stringify({ code: -1, msg: "未找到录像片段" })
                          );
                          // reject('未找到录像片段');
                        }
                      } else {
            ...
            if (playParams && playParams.env && playParams.env.domain) {
              isHttp =
                playParams.env.domain.indexOf("https") !== -1
                  ? "false"
                  : "true";
            } else {
              isHttp =
      ...
      getRealUrl.then(function () {
        ...
        _this.opt.sources.forEach(function (item, index) {
          if (getQueryString("dev", item) || item.indexOf("ws") !== -1) {
            ...
            if (validateCode) {
              _this.log(
                "设置秘钥，视频路数：" + (index + 1) + "验证码：" + validateCode
              );
            ...
            if (
              _this.playParams &&
              _this.playParams.hasOwnProperty("userName") &&
              _this.playParams.hasOwnProperty("password")
            ) {
              wsUrl = item.split("?")[0];
              wsParams = {
      ...
      addJs(jsPluginPath, function () {
        ...
        _this.jSPlugin
          .JS_SetOptions({
            ...
            bSupporDoubleClickFull:
              typeof playParams.isSupporDoubleClickFull === "undefined"
                ? true
                : playParams.isSupporDoubleClickFull, //是否双击窗口全屏，默认支持
            //bOnlySupportMSE: true  //只支持MSE
            bOnlySupportJSDecoder:
              typeof playParams.useMSE === "undefined" ||
              playParams.useMSE === false
                ? true
                : false, //是否强制使用jsdecoder
          })
          .then(function () {
